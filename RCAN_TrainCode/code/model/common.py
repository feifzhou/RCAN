...
def default_conv(in_channels, out_channels, kernel_size, bias=True, dim=2):
...
class BasicBlock(nn.Sequential):
    def __init__(
        ...
        bn=True, act=nn.ReLU(True), dim=2):

        m = [NNop[(dim,'conv')](
            in_channels, out_channels, kernel_size,
            padding=(kernel_size//2), stride=stride, bias=bias)
        ]
        if bn: m.append(NNop[(dim,'bnorm')](out_channels))
...
class Upsampler(nn.Sequential):
    def __init__(self, conv, scale, n_feat, bn=False, act=False, bias=True, dim=2):
